...
export function mergeDifferentVersionResult(datas: DOneLine[][], verses: VerseRange): DOneLine[] {
  ...
  const datas2a = LQ.from(datas2).where(a1 => a1.addresses !== undefined).toArray();
  const datas2b = LQ.from(datas2).where(a1 => a1.addresses === undefined).toArray();
  ...
  function getVersionOrder() {
    return LQ.from(datas).select(a1 => a1[0].ver).toArray();
  }
  function doEachVerseAndJoin() {
    const datas3a = LQ.from(addrDistinct).select(a1 => {
      const re1 = LQ.from(datas2a)
        .select((aa1, ii1) => ({ idx: ii1, da: aa1 }))
        .where(aa1 => isTheSameAddress(aa1.da.addresses.verses[0], a1))
        .toArray();

      // re1 很多的符合, 接著按版本順序呈現
      const re2 = LQ.from(versionOrder)
        .select(aa1 => LQ.from(re1)
          .firstOrDefault(aaa1 => aaa1.da.ver === aa1))
        .where(aa1 => aa1 !== undefined)
        .select(aa1 => aa1.da).toArray();

      // 移除掉, 下個 loop 會更快
      LQ.from(re1).select(aa1 => aa1.idx).reverse()
