...
export class AddReferenceCnv implements IAddBase {
  ...
  static reg2: RegExp;
  private static reg1GB: RegExp;
  static reg2GB: RegExp;
  constructor() { }
  main(lines: DOneLine[], verses: VerseRange): DOneLine[] {
    ...
    function makesureRegexExist() {
      if (DisplayLangSetting.s.getValueIsGB()) {
        if (AddReferenceCnv.reg1GB === undefined) {
          pthis.generateRegexGB();
        }
      } else {
    ...
    function getRegExp(): RegExp {
      return DisplayLangSetting.s.getValueIsGB() ? AddReferenceCnv.reg1GB : AddReferenceCnv.reg1;
    }
  }

  private cvtDescription(str: string, addrs: VerseRange) {
    str = str.replace(/參|参/g, '');
    str = str.replace(/；|，|。/g, ';');
    str = str.replace(/~/g, '-');
  ...
  private generateRegexGB() {
    const r1 = BibleBookNames.getBookNames(BookNameLang.太GB).join('|');
    const r2 = `(?:${r1})`;
    const partA = `(?:参?${r2}?\\d+:\\d+(?:~(?:\\d+:)?\\d+)?(?:；|，|。)?)`;
    const partB = `(?:参?(?:${r2})\\d+)`; // 參?(?:撒下|詩)\d+
    const r3 = `(?:${partA}|${partB})+`;
    AddReferenceCnv.reg1GB = new RegExp(r3, 'g');
    const r4 = `${r1}`;
    const partA2 = `(${r4})?(\\d+:\\d+(?:-(?:\\d+:)?\\d+)?)`;
    const partB2 = `(?:${r4})\\d+`;
    AddReferenceCnv.reg2GB = new RegExp(`${partA2}|${partB2}`, 'g');
    return;
  }
  private generateRegex() {
    ...
    const r1 = BibleBookNames.getBookNames(DisplayLangSetting.s.getBookNameLangWhereIsGB()).join('|');
    // console.log(r1);
    const r2 = `(?:${r1})`;
    // console.log(r2);
    // new RegExp(r2); // test
