...
describe('API-qsb.php', () => {
  ...
  it('qsb.php gb 繁簡', (done) => {
    const qstr = VerseRange.fromReferenceDescription('詩150', 40).toStringEnglishShort();
    const service = new ApiQsb();
    service.queryQsbAsync({ qstr, isSimpleChinese: false }).toPromise().then(a1 => {
      ...
      service.queryQsbAsync({ qstr, isSimpleChinese: true }).toPromise().then(a3 => {
        // tslint:disable-next-line: max-line-length
  ...
  it('qsb.php version', (done) => {
    const qstr = 'Mt1:1';
    const service = new ApiQsb();
    service.queryQsbAsync({ qstr, bibleVersion: 'recover' }).toPromise().then(a1 => {
  ...
  it('qsb.php strong', (done) => {
    const qstr = 'Mt1:1';
    const service = new ApiQsb();
    service.queryQsbAsync({ qstr, isExistStrong: true }).toPromise().then(a1 => {
      // tslint:disable-next-line: max-line-length
